...
st.markdown(
    """
    Análise comportamental utilizando técnicas de clustering
...
if uploaded_file is not None:
    ...
    if 'social_interaction_level' in df_model.columns:
        df_model['social_interaction_level'] = (
            df_model['social_interaction_level']
            .map({
                'low': 0,
                'medium': 1,
    ...
    st.markdown(
        """
        Valores próximos de 1 indicam melhor separação
        entre os grupos.
        """
    )
    ...
    st.markdown(
        f"""
        O valor do Silhouette Score obtido
        ({round(silhouette, 3)})
        indica baixa separação estrutural entre os clusters,
        sugerindo elevada sobreposição comportamental entre
        os indivíduos analisados.

        Os resultados demonstram que técnicas de clustering
        podem auxiliar na identificação de padrões
        comportamentais, embora apresentem limitações diante
        da complexidade do comportamento humano e da natureza
        multifatorial dos dados.
        """
    )
    ...
    st.info(
        "Faça upload do dataset para iniciar a análise."
    )
